...
internal class Program
{
    static void Main(string[] args)
    {
        ...
        using (clientSock) // 해당 블록을 나갈 경우 자동으로 clientSock.Close()처리
        {
            ...
            while (isOpened == true)
            {
                ...
                StringBuilder text = new StringBuilder(); // 전송할 데이터를 담는 텍스트 생성

                sendBuffer = Encoding.UTF8.GetBytes(text.ToString());
    ...
    private static void HandleLogin(string data)
    {
